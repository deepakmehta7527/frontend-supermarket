  ...
  var onlogin = async (e) => {
    try {
      ...
      var resp = await fetch(
        `${process.env.REACT_APP_APIURL}/login?un=${uname}&pass=${pass}`
      );
      if (resp.ok) {
        ...
        if (result.statuscode === 0) {
          ...
          if (rembme === true) {
            ...
            objcookie.set("usercookie",result.userdata._id,{maxAge: 60*60*24*7});
          ...
          toast.error(
            "Your account is not activated, please activate and then login"
          );
        }
      }
    } catch (e) {
