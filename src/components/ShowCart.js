...
var ShowCart = () => {
    ...
    var ondel = async (id) => {
        if (window.confirm("Are you sure to delete?")) {
            try {
                var resp = await fetch(`${process.env.REACT_APP_APIURL}/delcart/${id}`,
                    {
                        method: "delete"
                    })
                if (resp.ok) {
                    ...
                    if (result.statuscode == 1) {
                        toast.success("Show Cart deleted successfully");
                        fetchcart();
                    }
                }
                    else {
                        toast.success("Show Cart not deleted successfully");
                    }
