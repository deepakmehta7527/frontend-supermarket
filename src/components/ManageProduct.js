...
var ManageProduct = () => {
  ...
  const [times,settimes]=useState(1500);
  ...
  autoClose: times    // Adjust the duration in milliseconds (e.g., 3000 for 3 seconds)
})
          clearfields();
        } else {
          toast.success('product not added Successufully', {
            autoClose: times // Adjust the duration in milliseconds (e.g., 3000 for 3 seconds)
          })
        }
      }
  ...
  var fetchsubcat = async () => {
    try {
      var resp = await fetch(
        `${process.env.REACT_APP_APIURL}/fetchsubcat/${cat}`
      );
      if (resp.ok) {
        var result = await resp.json();
        if (result.statuscode === 0) {
  ...
  var fetchprodsbysubcat = async () => {
    try {
      var resp = await fetch(
        `${process.env.REACT_APP_APIURL}/fetchprodsbysubcat/${subcat}`
      );
      if (resp.ok) {
        var result = await resp.json();
        if (result.statuscode === 0) {
  ...
  var delprod = async (id) => {
    if (window.confirm("Are you sure to delete?")) {
      try {
        var resp = await fetch(
          `${process.env.REACT_APP_APIURL}/delprod/${id}`,
          {
            method: "delete",
          }
        );
        if (resp.ok) {
          ...
          toast.success("Category not deleted successfully");
        }
      } catch (e) {
          ...
          >
            <form name="form1" onSubmit={onsave}>
                ...
                {allcat.length > 0 ? (
                  <>
                    {allcat.map((data, indx) => (
                      <option value={data._id} key={indx}>
                        {data.catname}
                      </option>
                    ))}
                  </>
              ...
              <input
                ...
                name="discount"
                value={discount}
                placeholder="Discount(in percent, do not add percent symbol)"
